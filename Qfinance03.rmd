...
The ARCH/GARCH (autoregressive conditional heteroskedasticity / generalized autoregressive conditional heteroskedasticity) models are based on the homoskendasticity assumptions of the least squares models. The expected value of all squared error terms or variance is same at any given point. The opposite concept is heterskendasticity, where variance of the error term is not constant for all points. The ARCH/GARCH model uses the heterskendasticity as a measure to model variance. In financial time series like return on an asset or stock, the volatility occurs in clusters like periods of high volatility follows with high volatility and vice versa. ARCH/GARCH models are utilizes the heteroskedasticity in time series data to provide a volatility (standard deviation) measure. Volatility is important in assessing financial risk, portfolio selection, and derivative pricing. THE ARCH/GARCH models are applied to forecast mean and variance of return based on the past information. These models allow to use the best weights in forecasting variance instead of using equal weights for all past events. For financial time series more recent events would be more relevant then the distant past events.  
The GARCH model is typically described as GARCH(1,1) model. The(1,1) terms reefers to autoregressive lags or ARCH and moving average lags, respectively. This model is basically setup for one period forecast, but a long-horizon forecasting can be constructed. The GARCH equation is derived using the maximum likelihood using the software like R.
The ARCH/GARCH approaches are widely used in situations where volatility of return is a central issue. Finaclial institutes uses VaR as a way to measure their portfolio risk. 


http://www.cmat.edu.uy/~mordecki/hk/engle.pdf

Financial institutions measure the risk using a Value-at-Risk (VaR), usually calculated at the 99% confidence level. For stock proces, the low volatility period is followed by periods of high volatility and vice versa. This phenomenon is the base assumption in ARCH('Autoregressive Conditional Heteroskedasticity) model. A simple line plot of returns shows it.  We can test it by using two commonly uded tests(•The Ljung-Box test for autocorrelation in squared returns and the Lagrange Multiplier (LM)).
