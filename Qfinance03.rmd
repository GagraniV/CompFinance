...
```
## Finacial risk volatility modeling (ARCH, GARCH, & EGRCH) with R
...
Financial institutions measure the risk using a Value-at-Risk (VaR), usually calculated at the 99% confidence level. Generally, a stock prices shows the low volatility periods followed by periods of high volatility and vice versa. This phenomenon is visually detectable in simple line plots. In addtion,we can test it applying the two commonly used tests(The Ljung-Box test for autocorrelation in squared returns and the Lagrange Multiplier (LM)). In rugarch library there are diffrent functions for model specification, parameter estimation, backtesting, and forecasting. 
```{r}
