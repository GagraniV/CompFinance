...
Continous Componding, cc means that the principal is constsantly earning interest, and the interest keeps earning interest. The cc provides computaional adavantages(rc=ln(1+r)). Using the natural log and exponstial funcations, it  is easy to scale forward and discounting, scaling over multiple periods, as well as its time consitent. 
...
t.test(returns_lcc00[,"aapl"], alternative="two.sided",mu=0, conf.level=0.95)
...
```{r}
roll_muhat <- rollapply(returns_lcc[,"wfc"], width=24,FUN=mean, align="right")
#roll_muhat[1:5]
...
roll_rhohat <- rollapply(returns_lcc[,c("wfc","aapl")],
                       width=24,FUN=rhohat, by.column=FALSE,
                       align="right")
#roll_rhohat[1:5]
