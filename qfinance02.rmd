...
se_muhat_wfc = sigmahat_wfc/sqrt(length(wfc))
se_muhat_wfc
...
wfc_sd_boot = sd_boot(wfc_ret, statistic = sd_boot, R=999)
wfc_sd_boot
...
t_test_aapl = t.test(returns_lcc[,"aapl"], 
...
t.test(returns_lcc[,"wfc"], alternative="two.sided",
       mu=0, conf.level=0.95)
t.test(returns_lcc[,"sp500"], alternative="two.sided",
       mu=0, conf.level=0.95)
...
t.test(x=returns_lcc[1:50,"aapl"],
       y=returns_lcc[51:100,"aapl"],
       paired=F)
...
roll_rhohat = roll_rhohat = rollapply(returns_lcc[,c("wfc","aapl")],
                       width=24,FUN=rhohat, by.column=FALSE,
                       align="right")
class(roll_rhohat)
roll_rhohat[1:5]
...
Error in positions(x) : 
  trying to get slot "positions" from an object (class "zoo") that is not an S4 object 
