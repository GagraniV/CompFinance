...
class(wfc_lcc)
plot(wfc_lcc,reference.grid=F)


# compute rolling means of width 24 months 
roll_mean_24 = aggregateSeries(wfc_lcc,moving=23,adj=1,FUN=mean)
roll_mean_24
plot(roll_mean_24,reference.grid=F,
     main="Rolling 24 month mean estimates")
abline(h=muhat_vals["wfc"])

plot(roll_mean_24,wfc_lcc,reference.grid=F,
     main="WFC returns and rolling 24 month means")
abline(h=muhat_vals["wfc"])

# compute rolling sds of width 24 months 
roll_sd_24 = aggregateSeries(wfc_lcc,moving=23,adj=1,FUN=sd)
roll_sd_24
plot(roll_sd_24,reference.grid=F,
     main="Rolling 24 month SD estimates")
abline(h=sigmahat_vals["wfc"])

plot(roll_sd_24,wfc_lcc,reference.grid=F,
     main="WFC returns and rolling 24 month sds")
abline(h=muhat_vals["wfc"])
...
---
Boothstrapping is a resampling tchnique can be used to generate SE and confidence intervals without using the analytical formulas. This method doesnt need normally distrbuted data of a relatively large sample size. Hence it is widely used in CER model or VaR.
```{r}
...
muhat_wfc = mean(wfc_ret)
sigmahat_wfc = sd(wfc_ret)
se_muhat_wfc = sigmahat_wfc/sqrt(length(wfc_ret))
...
nobs = length(wfc_ret)
for (i in 1:B) {
  boot_data = sample(wfc_ret, nobs, replace=TRUE)
  muhat.boot[i] = mean(boot_data)
}

# bootstrap bias
mean(muhat_boot) - muhat_wfc
# bootstrap SE
sd(muhat_boot)
# analytic SE
sigmahat_wfc/sqrt(length(wfc_ret))

par(mfrow=c(1,2))
hist(muhat_boot, col="slateblue1")
abline(v=muhat_wfc, col="white", lwd=2)
qqnorm(muhat_boot)
qqline(muhat_boot)
par(mfrow=c(1,1))
# Boot mean estimates using R boot package function boot().  Boot requires user-supplied functions that take two arguments: data and an index. The index is created by the boot function and represents random resampling with replacement.

wfc_mean_boot = boot(wfc_ret, statistic = mean_boot, R=999)
class(MSFT_mean_boot)
names(MSFT_mean_boot)
# compare boot SE with analytic SE
se_muhat_wfc = sigmahat_MSFT/sqrt(length(wfc))
se_muhat_wfc
...
wfc_upper = muhat_wfcT + 2*se_muhat_wfc
cbind(MSFT_lower,MSFT_upper)
...
wfc_sd_boot = boot(wfc_ret, statistic = sd_boot, R=999)
wfc_sd_boot

# compare boot SE with analytic SE
se_sigmahat_wfc = sigmahat_wfc /sqrt(2*length(wfc))
se_sigmahat_wfc 
...
boot_ci(wfc_sd_boot, conf=0.95, type=c("norm", "basic", "perc"))
...
wfc_SP500_cor_boot = boot(returns_mat[,c("wfc","sp500")],
                           statistic=rho_boot, R = 999)
wfc_SP500_cor_boot
# compare boot SE with analytic SE based on CLT
se_rhohat_wfc_SP500 = (1 - rhohat_wfc_SP500^2)/sqrt(length(wfc))
se_rhohat_wfc_SP500
...
boot_ci(wfc_SP500_cor_boot, conf=0.95, type=c("norm", "basic", "perc"))

# boot estimate of normal distribution quantile

norm_quantile_boot = function(x, idx, p=0.05) {
# x_mat	data to be resampled
...
wfc_q05_boot = boot(wfc, statistic=norm_quantile_boot, R=999)
wfc_q05_boot
plot(wfc_q05_boot)
boot_ci(wfc_q05_boot, conf=0.95, type=c("norm", "basic", "perc"))
...
wfc_VaR_boot = boot(wfc, statistic = ValueAtRisk_boot, R=999)
wfc_VaR_boot
boot_ci(wfc_VaR_boot, conf=0.95, type=c("norm", "perc"))
...
nobs = dim(returns_lcc)[1]
muhat_vals = apply(returns_lcc, 2, mean)
muhat_vals
sigmahat_vals = apply(returns_lcc, 2, sd)
se_muhat = sigmahat_vals/sqrt(nobs)
se_muhat
...
t_test_appl = t.test(returns_lcc[,"appl"], 
...
t_test(returns_lcc[,"wfc"], alternative="two.sided",
       mu=0, conf.level=0.95)
t.test(returns_lcc[,"sp500"], alternative="two_sided",
       mu=0, conf.level=0.95)
...
par(mfrow=c(2,2))
	hist(returns_lcc[,"wfc"],main="wfc monthly cc returns",
	probability=T, ylab="cc return", col="slateblue1")
	boxplot(returns_lcc[,"wfc"],outchar=T, ylab="cc return",
              col="slateblue1")
	plot(density(returns_lcc[,"wfc"]),type="l",xlab="cc return",
	     ylab="density estimate", main="Smoothed density",
           lwd=2, col="slateblue1")
	qqnorm(returns_lcc[,"wfc"], col="slateblue1")
	qqline(coredata(returns_lcc[,"wfc"]))
par(mfrow=c(1,1))

wfc_skew = skewness(returns_lcc[,"wfc"])
wfc_ekurt= kurtosis(returns_lcc[,"wfc"])			# note: this is excess kurtosis
...
library(tseries)
jarque.bera.test(returns_lcc[,"wfc"])

#
# test for no autocorrelation in returns
#

wfc.acf = acf(returns_lcc[,"wfc"])
...
t.test(x=returns_lcc[1:50,"appl"],
       y=returns_lcc[51:100,"appl"],
       paired=F)
...
roll_muhat = rollapply(returns_lcc[,"wfc"], width=24,
                       FUN=mean, align="right")
class(roll_muhat)
roll_muhat[1:5]

# plot rolling estimates with data
plot(merge(roll_muhat,returns_lcc[,"wfc"]), plot.type="single",
...
roll_sigmahat = rollapply(returns_lcc[,"wfc"],width=24,
                          FUN=sd, align="right")
roll_sigmahat[1:5]

# plot rolling estimates with data
plot(merge(roll_sigmahat,returns_lcc[,"wfc"]), plot.type="single",
...
# compute rolling means over 24 month windows
roll_muhat = rollapply(returns_lcc[,"sp500"], width=24,
                       FUN=mean, align="right")
class(roll_muhat)
roll_muhat[1:5]

# plot rolling estimates with data
plot(merge(roll_muhat,returns_lcc[,"sp500"]), plot.type="single",
...
# compute rolling standard deviations over 24 month windows
roll_sigmahat = rollapply(returns_lcc[,"sp500"],width=24,
                          FUN=sd, align="right")
roll_sigmahat[1:5]

# plot rolling estimates with data
plot(merge(roll_sigmahat,returns_lcc[,"sp500"]), plot.type="single",
...
roll_rhohat = rollapply(returns_lcc[,c("sp500","wfc")],
...
roll_rhohat = roll_rhohat = rollapply(returns_lcc[,c("wfc","appl")],
                       width=24,FUN=rhohat, by.column=FALSE,
                       align="right")
class(roll_rhohat)
roll_rhohat[1:5]

plot(roll_rhohat, main="Rolling Correlation b/w wfc and appl",
