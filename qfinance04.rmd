...
```{r}
# wfc_ret is the continous compounding returns from montly prices, returns are stationary.coredata function eliminates any index and provides the data field only.

wfc_ret1 <- coredata(wfc_ret)
 #  the wfc_sim_pret is simple monthly(%) return.
 
wfc_sim_pret[which.min(wfc_sim_pret)]
# The largest one month loass for wfc was -36%.

plot(wfc_sim_pret,main="MOnthly Simple Return of WFC", xlab = "Days",ylab="Return in %")
...
##For GARCH Model, we used the below function from the rugarch library. 
