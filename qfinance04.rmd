...
The Vector autoregressive models (VAR) are the multivariate extensions of the univariate autoregressive (AR) models, captures the linear dependencies among multiple time series. As per the VAR model each variable is evolved through a linear function of all other lagged variables. All varibles depends on the past values of its own as well as the other varibles in consideration. For a VAR model, all variables have to be stationary or show the same order of intgration. In case if the varibles are nonstationary, or cointegrated, the error correction term is included in the VAR and the model is kknown as VECM or Vector Error Correction Model. 

A nonstationary time series have means, variances, and covariances that change over time. It occurs as trends, cycles, random walks, or combinations of the three. Regression models without accounting the nonstationarity results into spuriuos regressions. The cointegration technique is useful to investigate cointegrated or nonstationary variables important to avoid the spurious regression. The Engle-Granger tests and the Johansen-Juselius tests allow to test the cointegration.
Augmented Dickey–Fuller (ADF) t-statistic test: small p-values suggest (library(urca))
A VAR model is primarly used to assess the Impulse-response from a stress. Essentially, an impulse-response function shows how a variable reacts (response) to a shock (impulse) influsing any other variable in the system. For example the impulse respose of 1SD shock in S&P prices for a univariate analysis. For multivatiate analysis, covsariance matrix is used.
...
The primary purpose is to make a forecast for the S&P index by using the additional variables and to identify impulse responses. The above three varible holds a long term relationship. 
