...
The goal of this repositoy is to explore the econometric models and methods for the multiple return finacial time series known as the vector or multivariate time series analysis.
...
A process in which statistical properties do not changes for a shift in time. In a strict stationary time series all moments of all degreesare stays same. However, for general pupose a weak defination is used, where only two moments(means and variance) unchaged for a shift in time, and autocorrelation/covariance only depends on the lag.
