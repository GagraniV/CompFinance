...
Statistical properties of the for the stationary time series do not changes for a shift in time. In a strict stationary time series all moments of all degrees stays same. However, for general pupose a weak defination is used, where only two moments(means and variance) unchaged for a shift in time, and autocorrelation/covariance only depends on the lag.

The stylized facts are statistical propertis which stays consitent over a time period for a type of a dataset. For finacial time series such as returns often exhibits fat tails and the autocorrelation. 
