...
This repository focuses on to explore the econometric models and methods to analyze multiple finacial time series of returns, known as the vector or multivariate time series analysis models.
...
Statistical properties of the for the stationary time series do not changes for a shift in time. In a strict stationary time series all moments of all degrees stays same. However, for general pupose a weak defination is used, where only two moments(mean and variance) unchaged for a shift in time(or mean reversion), and autocorrelation/covariance only depends on the lag length.
