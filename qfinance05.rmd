...
WFC <- WFC[, "WFC.Adjusted", drop=F]
GSPS <- GSPC[, "GSPC.Adjusted", drop=F]

data<-merge(
  WFC,
  GSPS,
  DTB3
)["1994-03-01::2015-08-30"]
...
#Reduced VAR model
var1 <- VAR(dataDaily, lag.max=4, type="both", ic="AIC")
...
acf(residuals(var1)[,3])
acf(residuals(var1)) # cross correlation matrix for all residuals


#Granger Causality. NULL: No GC. Rejectining null hypothesis shows that the varibles have an interdependency and should be included.
GC can be bivariate or multivariate.
